    ...
    def position_embeding(self, img_feats, calibs, pad_h, pad_w, masks=None):
        ...
        with torch.no_grad():
            ...
            for extrinsic, intrinsic, fov in zip(calibs['extrinsic_matrix'], calibs['intrinsic_matrix'], calibs['fov']): 
                ...
                K_inv_all.append(torch.from_numpy(K_inv).to(img_feats.device).to(dtype))
                extrinsics_inv_all.append(torch.from_numpy(extrinsics_inv).to(img_feats.device).to(dtype))
            ...
            px_coords = torch.from_numpy(px_coords[:, None]).to(dtype).to(img_feats.device)

            depths=np.array([self.depth_start + (depth_index*self.depth_step) for depth_index in range(self.depth_num)])
            depths=torch.from_numpy(rearrange(depths, 'D -> 1 D 1')).to(dtype).to(img_feats.device)
            ...
            if 0:
                ...
                for cam_ in cams:
                    ...
                    extrinsics_inv = np.linalg.inv(viewMatrix)
                    K_inv_torch = torch.from_numpy(K_inv).to(img_feats.device).to(dtype)
                    extrinsics_inv_torch = torch.from_numpy(extrinsics_inv).to(img_feats.device).to(dtype)
            ...
            coords3ds = inverse_sigmoid(coords3ds)
        coords3ds_clone = coords3ds.clone().detach()
        coords_position_embeding = self.position_encoder(coords3ds_clone)
