...
from robouniview.models.action_head import DeterministicDecoder, DiffusionDecoder, FCDecoder, GPTDecoder, TokenFCDecoder, Multi_Action_Token_FCDecoder, Multi_Action_Token_FCDecoder_for_keyframe, DiffusionActorDecoder
    ...
    def __init__(
        ...
        if decoder_type == 'lstm':
            ...
            if use_hist:
                ...
                self.lang_encoder.lm_head = self.action_head = Multi_Action_Token_FCDecoder_for_keyframe(in_features, self.window_size, 
            ...
            if use_diff:
                ...
                raise NotImplementedError
        elif decoder_type == 'diffusion_actor':
            self.lang_encoder.lm_head = self.action_head = DiffusionActorDecoder(
                in_features, 
                self.window_size,
                input_dim=4, # ????
                n_timesteps=n_timesteps,
    ...
    def forward(
        ...
        if self.train_action:
            ...
            if self.args.action_token or self.args.multi_action_token:
                output_hs = self.lm_head(output_hs, state_tensor=state_tensor, action_mask = action_mask, gt_trajectory=labels)
            else:
