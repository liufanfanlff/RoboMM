...
"""
Pycls models utilities.
"""

import os

try:
    import pycls  # pylint: disable=unused-import # noqa: F401
except ImportError:
    os.system("pip install pycls")
...
_MODEL_ZOO_CONFIGS = {
    ...
    "RegNetX-950MF": "dds_baselines/regnetx/RegNetX-950MF_dds_8gpu.yaml",
    "RegNetX-1.4GF": "dds_baselines/regnetx/RegNetX-1.4GF_dds_8gpu.yaml",
...
def get_config_file(name):
    ...
    assert name in _MODEL_ZOO_CONFIGS, err_str
...
def get_weights_file(name):
    ...
    err_str = f"Model {name} not found in the model zoo."
    assert name in _MODEL_ZOO_WEIGHTS, err_str
    if _MODEL_ZOO_WEIGHTS[name].startswith("dds_baselines"):
        weights_url = os.path.join(_URL_WEIGHTS, _MODEL_ZOO_WEIGHTS[name])
        return cache_url(weights_url, _DOWNLOAD_CACHE, _URL_WEIGHTS)
    else:
        return _MODEL_ZOO_WEIGHTS[name]
