...
def train_one_epoch_calvin(
    ...
    for num_steps, batch_calvin in t:
        ...
        calib = batch_calvin[6] # YF: 0.image;1.text;2.action;3.gripper; 4.state;5(static extr;static intr;static_dist...;static fov, 'gripper_cam2gripper'),6:pcd;7:index, 
        ...
        if ((num_steps + 1) % args.logging_steps == 0) and args.rank == 0:
            if args.multi_action_token:
                final_pos_l2 = ((num_actions[..., :3] - labels[0][:, :args.window_size, :3]) ** 2).sum(-1)[...,-1].sqrt().detach()
                pos_l2_final = final_pos_l2.mean()
                pos_l2_final05 = (final_pos_l2 < 0.05).to(torch.float).mean()
                l1 = (num_actions[..., 3:] - labels[0][:, :args.window_size, 3:]).abs().sum(-1)[...,-1].detach()
                rot_l1 = l1.to(torch.float).mean()
                rot_l105 = (l1 < 0.05).to(torch.float).mean()
                rot_l1025 = (l1 < 0.025).to(torch.float).mean()
                gripper_acc = ((bin_actions > 0.5).to(torch.int) == (labels[1][:, :args.window_size, :]>0.5).to(torch.int)).to(torch.float).mean().detach()
                print(f"pos_l2_final:{pos_l2_final:.3f}, pos_l2_final05:{pos_l2_final05:.3f}, rot_l1:{rot_l1:.3f}, rot_l105:{rot_l105:.3f}, rot_l1025:{rot_l1025:.3f}, gripper_acc:{gripper_acc:.3f}")
...
def train_one_epoch_keyframe(
    ...
    for num_steps, batch_calvin in t:
        ...
        # compute loss
        if args.multi_action_token:
            if 'diffusion' in model.module.decoder_type:
                loss_calvin_num, loss_calvin_bin = output.logits, torch.zeros_like(output.logits)
            else:
                num_actions, bin_actions = output.logits[0], output.logits[1]
                # reshape for loss calculation
                if args.multi_step_action != 1:
...
def train_one_epoch_move(
    ...
    for num_steps, batch_calvin in t:
        ...
        if args.multi_action_token:
            ...
            if 'Temporal' in args.fusion_mode:
                loss_calvin_num = torch.nn.functional.huber_loss(num_actions, labels[0][:, :args.window_size, :])
                num_classes = bin_actions.shape[-1]
                loss_calvin_bin = torch.nn.functional.binary_cross_entropy(bin_actions, torch.nn.functional.one_hot(labels[1][:, :args.window_size, 0].to(torch.int64), num_classes=num_classes).to(torch.float32))
            else:
                assert False
                loss_calvin_num = torch.nn.functional.huber_loss(num_actions, labels[0])
                loss_calvin_bin = torch.nn.functional.binary_cross_entropy(bin_actions, labels[1])
    ...
    for num_steps, batch_calvin in t:
        ...
        if 0:
            with torch.no_grad():
                ...
                o3d.io.write_point_cloud("/workspace/RoboFlamingo/pcd.pcd", pcd)
                o3d.io.write_point_cloud("/workspace/RoboFlamingo/pcd_true.pcd", pcd_true)
