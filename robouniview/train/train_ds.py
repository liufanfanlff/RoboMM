...
from open_flamingo.train.distributed import init_distributed_device_with_ds, world_info_from_env
...
get_ckpt_name, get_ckpt_name_pattern, train_one_epoch_move
...
def main():
    ...
    parser.add_argument("--config", type=str, default=None, help="path to checkpoint to resume from, this should contain model, optimizer, and lr_scheduler states",)
    parser.add_argument("--save_dir", type=str, default=None, help="save_dir")
    parser = deepspeed.add_config_arguments(parser)  # 添加 DeepSpeed 特定参数
    ...
    device_id = init_distributed_device_with_ds(args)
    ...
    # 使用 DeepSpeed 初始化模型
    ddp_model, optimizer, _, _ = deepspeed.initialize(
        args=args,
        model=model,
