...
sys.path.append('/project/robotic/robo_mm/open_flamingo')
import argparse
import copy
import glob
import os
...
os.environ['TORCH_HOME']='/project/robotic/modelzoo/'
...
get_ckpt_name, get_ckpt_name_pattern, train_one_epoch_move, train_one_epoch_keyframe
...
def main():
    ...
    calvin_dataset = get_data(args, image_processor, tokenizer, 'keyframe' if 'keyframe' in args.llm_name else '')
    ...
    total_training_steps = (len(calvin_dataset.dataset) // (args.batch_size_calvin * args.world_size)) * args.num_epochs # 弃用(args.train_num_samples_calvin)
    if args.rank == 0:
    ...
    for epoch in range(resume_from_epoch, args.num_epochs):
        ...
        elif 'keyframe' in args.llm_name:
...
if __name__ == "__main__":
    ...
    # new_state_dict = rename_state_dict_keys(new_state_dict, 'occ_decoder.', 'occ_decoder_UVFormer.')
    # new_state_dict = rename_state_dict_keys(new_state_dict, 'Upsample2d_3d.', 'Upsample2d_3d_UVFormer.')
    # new_state_dict = rename_state_dict_keys(new_state_dict, 'bevformer.', 'uvformer.')
