...
from robouniview.data import keyframe_data
import functools
...
class ModelWrapper(CalvinBaseModel):
    ...
    def step(self, obs, goal,env, get_action=True):
        ...
        if self.model.module.use_state or self.model.module.sep_lm_head:
            state = np.concatenate([obs['robot_obs'][:3], keyframe_data.convert_rotation(obs['robot_obs'][3:6]), (obs['robot_obs'][[-1]] + 1) / 2], axis=-1) # obs['robot_obs']     # Map gripper openess to [0, 1]
        ...
        # Back to quaternion
        action = keyframe_data.unconvert_rot(torch.as_tensor(action)).numpy()
        # unnormalize position
        gripper_loc_bounds = np.array([[-0.5, -0.5, 0.0], [0.5, 0.5, 1]])
        action[..., :3] = keyframe_data.unnormalize_pos(action[..., :3], gripper_loc_bounds)
...
def rollout(env, model, task_oracle, subtask, val_annotations, plans, debug, eval_log_dir='', subtask_i=-1, sequence_i=-1, robot_obs=None, scene_obs=None, diverse_inst=False):
    ...
    for step in range(EP_LEN):
        ...
        def convert_action(trajectory):
            ...
            _rot = keyframe_data.convert_quaternion_to_euler(rotation)
            ...
            if (_rot != _rot).any():
                ...
                rotation = np.array([
                    pb.getEulerFromQuaternion([r[-1], r[0], r[1], r[2]])
                    for r in _rot
                ]).reshape(_rot_shape)
            else:
                rotation = _rot
        ...
        for act_ind in range(len(action)):
            ...
            curr_proprio = obs['robot_obs']
            obs, _, _, current_info = env.step(curr_action)
            obs['robot_obs'] = curr_proprio
