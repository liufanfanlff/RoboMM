...
class ModelWrapper(CalvinBaseModel):
    ...
    def step(self, obs, goal,env, get_action=True):
        ...
        return action, obs_preds_rgb_img, obs_preds_gripper_img, obs_pred
...
def rollout(env, model, task_oracle, subtask, val_annotations, plans, debug, eval_log_dir='', subtask_i=-1, sequence_i=-1, robot_obs=None, scene_obs=None, diverse_inst=False):
    ...
    for step in range(EP_LEN):
        ...
        if 1:
            # target_action = env.robot.target_orn + action[3:6] * 0.05
            # robot_obs_ort = np.array(pybullet.getMatrixFromQuaternion(pybullet.getQuaternionFromEuler(env.robot.target_orn))).reshape(3,3)
            # actions_ort = np.array(pybullet.getMatrixFromQuaternion(pybullet.getQuaternionFromEuler(target_action))).reshape(3,3)
            # B = np.array([
            #     [0, 1, 0],
            #     [-1, 0, 0],
            #     [0, 0, 1]
            ...
            # action = np.array([*action[:3], *delta_ort, action[-1]])
            # action = np.array([action[1], -action[0], action[2], action[3], action[4], action[5], action[6]])
            ...
            delta = action[3:6]
            current_orientation = np.array(pybullet.getMatrixFromQuaternion(pybullet.getQuaternionFromEuler(env.robot.target_orn))).reshape(3,3) 
            delta *= 0.05
            quat_error = robosuite_trans.axisangle2quat(delta)
            rotation_mat_error = robosuite_trans.quat2mat(quat_error)
            B = np.array([
                [0, 1, 0],
                [-1, 0, 0],
                [0, 0, 1]
            ...
            goal_ort = np.array(pybullet.getEulerFromQuaternion(rotation_matrix_to_quaternion(goal_orientation)))
...
if __name__ == "__main__":
    ...
    def angle_between_angles(a, b, max_orn=0.05):
        ...
        rel_orn = np.clip(rel_orn, -max_orn, max_orn) / max_orn
        return rel_orn
    a[3:6] = angle_between_angles(env.robot.target_orn, goal_ort, max_orn=0.05)
    a[:3] = np.clip(a[:3] * 0.02, -0.02, 0.02) / 0.02
