...
# import pyrender
sys.path.append("/project/robotic/LIBERO/")
import argparse
...
def to_relative_action(actions, robot_obs, max_pos=0.02, max_orn=0.05):
    ...
    pb.getEulerFromQuaternion(robot_obs[3:7])
...
def generate_data(args):
    ...
    descriptions = []
    shape_meta = None
    abs_datasets_dir = '/data/robotics/LIBERO'
    ...
    for task_id in task_sequences:
        ...
        for demo in range(len(demo_length) - 1):
            ...
            for step in range(demo_length[demo], demo_length[demo + 1]):
                if step == demo_length[demo]:
                    env.set_init_state(dataset.__getitem__(step)['states'][0])
                
                action = dataset.__getitem__(step)['actions'][0]
                env.env.done = False
                cur_obs, reward, done, info = env.step(action, isbreak=False)

                cache_pose.append(cur_obs)
            
            
            step = demo_length[demo]
            states = dataset.__getitem__(step)['states'][0]
            cur_obs = env.set_init_state(states)

            # env.env.model_timestep = 0.000001 #  0.002

            try:
                issucceed = False
                for step in range(len(cache_pose)):
                    ref_obs = cache_pose[step]
                    
                    step = demo_length[demo] + step

                    cur_pose = cur_obs['robot0_eef_pos']
                    ref_pose = ref_obs['robot0_eef_pos']
                    cur_quat = cur_obs['robot0_eef_quat']
                    ref_quat = ref_obs['robot0_eef_quat']

                    # calvin_action = np.array([*ref_pose, *list(pb.getEulerFromQuaternion(ref_quat)), 0])
                    # calvin_robot_obs = np.array([*cur_pose, *list(pb.getEulerFromQuaternion(cur_quat)), 0])
                    # action = to_relative_action(calvin_action, calvin_robot_obs, 1, 1)
                    # ref_pose2 = cur_pose + action[:3] * 1
                    # ref_quat2 = np.array(pb.getQuaternionFromEuler(action[3:6] + np.array(pb.getEulerFromQuaternion(cur_quat))))
                    # if ref_quat2[0] < 0:
                    #     ref_quat2 = -1*ref_quat2
                    # # print(ref_pose, ref_pose2, ref_quat, ref_quat2)
                    # ref_pose = ref_pose2
                    # ref_quat = ref_quat2

                    # delta_pos = (ref_pose - cur_pose)/0.05
                    # delta_ori = Trobosuite.quat2axisangle(Trobosuite.mat2quat(np.dot(Trobosuite.quat2mat(ref_quat), Trobosuite.quat2mat(cur_quat).T)))/0.5
                    # action = np.array([*delta_pos, *delta_ori, dataset.__getitem__(step)['actions'][0][-1]])

                    action = dataset.__getitem__(step)['actions'][0]
                    # print(action)
                    
                    # camera_dict = data_infos[0]
                    ...
                    # camera_dict['cur_obs'] = cur_obs
                    # camera_dict['ref_obs'] = ref_obs
                    # camera_dict['action_mat_delta'] = action

                    # np.savez(demo_path / f"{step - demo_length[demo]}.npz", **camera_dict)

                    env.env.done = False
                    cur_obs, reward, done, info = env.step(action, isbreak=False)
                    # print(f"ref:{ref_obs['robot0_eef_pos']}, cur:{obs['robot0_eef_pos']}, delta:{ref_obs['robot0_eef_pos']-obs['robot0_eef_pos']}")
                    # check if current step solves a task
                    if done:
                        issucceed = True
                    # def pose_error(cur_pose, ref_pose, cur_mat, ref_mat):
                    #     pos_err = np.linalg.norm(cur_pose - ref_pose)
                    #     rot_err = np.arccos(0.5 * (np.matmul(cur_mat, ref_mat.transpose(0, 1)).trace() - 1))
                    #     return pos_err, rot_err
                    
                    # pose_err = pose_error(cur_obs['robot0_eef_pos'], ref_pose, Trobosuite.quat2mat(cur_obs['robot0_eef_quat']), Trobosuite.quat2mat(ref_quat))
                    pose_err = [np.linalg.norm(cur_obs['robot0_eef_pos']-ref_pose), np.linalg.norm(orientation_error(Trobosuite.quat2mat(cur_obs['robot0_eef_quat']), Trobosuite.quat2mat(ref_quat)))]
                    delta_pose.append(pose_err)
                    # print(pose_err)
            except:
                pass
            print(issucceed)
        ...
        print(f"{cnt}/{len(demo_length) - 1}={cnt/(len(demo_length) - 1)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str,
                        default='/data/robotics/libero_uniview_tmp')
