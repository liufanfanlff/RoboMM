...
abs_datasets_dir = Path('/data/robotics/RLBENCH3D/raw_highres/train')
    ...
    for num_demo in os.listdir(task_path):
        ...
        for file_idx in range(len(frames)-1):
            ...
            try:
                ...
                robot_obs_ort = np.array(pybullet.getMatrixFromQuaternion(frames[file_idx].gripper_pose[3:7])).reshape(3,3)
                actions_ort = np.array(pybullet.getMatrixFromQuaternion(frames[file_idx+1].gripper_pose[3:7])).reshape(3,3)
                ...
                delta_pos = frames[file_idx+1].gripper_pose[:3] - frames[file_idx].gripper_pose[:3]
                action = np.array([*delta_pos, *delta_ort,  frames[file_idx].gripper_open])
...
np.save('data_folder/rlbench_cpmm_actions.npy', actions)
for i in range(3):
